...
void BPrintImage(const graph &X,const int delay_s, const int row_size, const int col_size)
{
    ...
    Write<<<(X_in->total+THREADSPERBLOCK-1)/THREADSPERBLOCK, THREADSPERBLOCK>>>(X->output, X_in->output, X_in->total);
    CheckError("Memcpy");
    BMinMaxNorm(X_in); // Forces image to be min max normed between 0 and 255;
...
Ipointer i2p(const std::string& filepath, int row_size, int col_size) 
{
    ...
    if (row_size > 0 && col_size > 0) { cv::resize(img, img, cv::Size(col_size, row_size), 0, 0, cv::INTER_AREA);}
...
Ipointer Bi2p(const str& folder, const int num_images, const int row_size, const int col_size) 
{
    ...
    cv::Mat img = cv::imread(folder+"/"+filepaths[0], cv::IMREAD_COLOR);
    Ipointer result;
    result.dimensions[0] = num_images;
    result.dimensions[1] = img.channels(); 
    result.dimensions[2] = row_size;
    result.dimensions[3] = col_size;
    result.labels = filepaths;

    float* P = (float*)malloc(img.channels()*num_images*row_size*col_size*sizeof(float));
    if (!P) { std::cerr << "Memory allocation failed\n"; std::exit(1);}
...
graph Bi2n(str filepath,const int num_images, const int row_size , const int col_size)
{
    ...
    for(auto & name : X.labels){std::cout << name << "\n";}
...
cv::Mat p2i(Ipointer X)
{
    ...
    int cvType = (channels == 1) ? CV_8UC1 : CV_8UC(channels);
    cv::Mat img(rows, cols, cvType);

    for (int ch = 0; ch < channels; ch++) {
    for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
        char pixel = (uchar)X.memory[ch * img.rows * img.cols + r* img.cols + c];
        if (img.channels() == 1) {img.at<uchar>(r, c) = pixel;}       
        else {img.at<cv::Vec3b>(r, c)[ch] = pixel;}  
        }}}
...
cv::Mat n2i(const graph X)
{
    ...
    for (int r = 0; r < rows; ++r) {
    for (int c = 0; c < cols; ++c) {   
        uchar pixel = cpu_img[ch * img.rows * img.cols + r* img.cols + c];
        if (img.channels() == 1) {img.at<uchar>(r, c) = pixel;}       
        else {img.at<cv::Vec3b>(r, c)[ch] = pixel;}  
    }}}
...
cv::Mat Bn2i(const graph X)
{
    ...
    cudaMemcpy(cpu_img, X->output, X->total * sizeof(float), cudaMemcpyDeviceToHost);
    int cvType = (channels == 1) ? CV_8UC1 : CV_8UC(channels);
    cv::Mat img(rows, cols * batch, cvType, cv::Scalar(0));
    ...
    for (int c = 0; c < cols; ++c) {
        ...
        if (channels == 1) {img.at<uchar>(r, b * cols + c) = val;}          
        else {img.at<cv::Vec3b>(r, b * cols + c)[ch] = val;}
    }}}}
    free(cpu_img);
    return img;
}
